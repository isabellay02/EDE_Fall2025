...
```{r, data.summary, echo = FALSE, eval=TRUE, message=FALSE, warning=FALSE}
...
# Create PM2.5 concentration table using kable function
PM25_table <- summary_PM25 %>%
  mutate(mean_PM2.5 = round(mean_PM2.5, 2)) # rounding data to two decimals places
knitr::kable(PM25_table,
             col.names = c("County", "$\\mu g/m^3$"),
             caption = "Mean Particulates (2.5mm)")

# Create Ozone concentration table using kable function
Ozone_table <- summary_Ozone %>%
  mutate(mean_Ozone = round(mean_Ozone, 2)) # rounding data to two decimals places
knitr::kable(Ozone_table,
             col.names = c("County", "$\\mu g/m^3$"),
             caption = "Mean Ozone")
```
...
```{r, 2018plots, echo = FALSE, fig.cap = "2018 monthly Ozone level", message=FALSE, warning=FALSE, fig.align='left', fig.height= 3}

#load packages
library(dplyr)
library(ggplot2)

# Use ggplot to create boxplot
ozonelevel_2018 <- epa_data %>%
  mutate(Month = factor(Month, levels = 1:12, labels = month.abb)) %>% # mutating month from numbers to characters
  filter(Year == 2018) %>% # filter out 2018 data
...
```{r, 2019plots, echo = FALSE, fig.cap = "2019 monthly Ozone level", message=FALSE, warning=FALSE, fig.align='left', fig.height= 3} 
#load packages
library(dplyr)
library(ggplot2)

# Use ggplot to create boxplot
ozonelevel_2019 <- epa_data %>%
  mutate(Month = factor(Month, levels = 1:12, labels = month.abb)) %>% # mutating month from numbers to characters
  filter(Year == 2019) %>% # filter out 2019 data
  ggplot(aes(x = Month, y = Ozone)) +
  geom_boxplot()+
  labs(title = "Monthly Ozone Concentration (2019)", 
       x = "Month", 
       y = "Concentration (ug/m^3) ")+
  theme()
ozonelevel_2019
...
## Git Repository
Github for Isabella Yang <https://github.com/isabellay02/EDE_Fall2025>
